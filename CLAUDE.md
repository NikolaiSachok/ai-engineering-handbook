...
- **Sidebar CATEGORY labels: `current.json` is the ONLY place a localized label lives.** Two files, two jobs,
  no third copy:
  - **Structure** — `position` and `link` — comes from the **EN `_category_.json`** in `docs/` /
    `docs-ai-sdlc/`. There is exactly one per category and it decides for **every** locale.
  - **The localized label** goes in `i18n/<locale>/docusaurus-plugin-content-docs*/current.json` under
    `sidebar.<sidebarId>.category.<English label>` (the EN label is the lookup key), alongside any
    `…link.generated-index.description`.
  - **Never create a `_category_.json` under `i18n/`.** Docusaurus overrides it with `current.json` at render
    time, so such a file **does not render at all** — editing it changes nothing, and nothing warns or fails.
    It is a decoy that costs a future editor an hour. (Measured in #290: all 58 locale copies deleted, all
    three locales rebuilt, generated sidebar trees **byte-identical** — labels, hrefs, nesting, `link`
    objects, `generated-index` descriptions. They contributed nothing.)
  (Regression source for the `current.json` half of the rule: the EN-canonical flip left every RU category
  label English because the labels were only in `_category_.json`.)
...
- **One** `_category_.json`, in `docs/` (EN), points at it: `{"label": "…", "position": N, "link": {"type":
  ...
  out of the child list (no duplicate). That single EN file decides `position` and `link` for **every**
  locale; the other locales get only a **label**, in their `current.json`. Do **not** mirror the
  `_category_.json` into `i18n/` — a locale copy is overridden by `current.json` and never renders, so
  editing one silently does nothing (see the locale-model rule at the top of this file).
...
- **Nest via folder + index — the base becomes the group index, so its URL is preserved.** A deepened lesson
  ...
  and a **single** `_category_.json` in `docs/` whose **`label`** is the lesson's topic in English,
  **`position`** = the base's old `sidebar_position`, and **`link: {type: doc, id: part-N/<lesson>/index}`**
  so clicking the group label opens Часть 1. Mirror the folder's **pages** in every locale; the
  `_category_.json` itself is **not** mirrored — each locale's label goes in its `current.json` under
  `sidebar.<sidebarId>.category.<English label>`, which is the only copy that renders. The autogenerated sidebar then shows a **collapsible group** (the topic) whose label opens Часть 1

...
Why this matters for agents is the AI-delta again. Agent runs are long, non-deterministic, and expensive — a thirty-step research agent that dies at step 28 must not re-pay for 27 model calls to get back. Durability is also what makes human-in-the-loop real: an interrupt node persists the state and the run simply waits — hours, days, across restarts and deploys — for someone to approve. What it does not do is pick up mid-node. On approval the interrupted node **executes again from its start**, so every line before the `interrupt()` call runs a second time — and no `durability` mode changes that, because the modes decide when state is written, not where execution picks up. So keep paid or irreversible work out of a node that interrupts: put the charge in its own node ahead of the pause, or make it idempotent under a key that survives the replay — the [idempotency-key discipline](../tool-use/deep-dive.md) from tool use, applied to your own graph. That's Part 1's HITL node, grounded in the same persistence. The general human-in-the-loop and budget layer is [planning & loops](../planning-loops/index.md); here it's just a node backed by a checkpoint.
...
## One artefact, every production concern
...
It all converges on one object. The graph you defined in the first section is the same one you checkpoint, remember with, define declaratively or not, and now trace and grade. One artefact, and every production concern hangs off it. That's the deepest form of Part 1's AI-delta: the graph isn't just something you can inspect — it's the single **seam** through which persistence, memory, and observability all attach.
...
- A checkpointer saves state at every step keyed by a `thread_id`, so a run resumes from the last successful step instead of restarting — and that same persistence is what lets a human-in-the-loop interrupt wait hours before re-running its node from the start. The `durability` modes (`exit`, `async`, `sync`) trade speed for safety; backends swap freely (in-memory for dev, Postgres or Redis for prod); a stateless single-shot agent needs none of it.
...
**[New terms](../../glossary.md#orchestration-frameworks)**: state graph, checkpointer, checkpoint backend, thread (thread_id), durable execution, conditional edge, framework long-term memory (store), declarative vs imperative agent definition.

...
один и тот же путь `retrieve → generate`. Один поиск, одна генерация, конец. Пайплайн не смотрит на запрос
...
Ровно тот сдвиг, о котором урок: как retrieval вырастает из простого поиска в агентную систему.
...
- **Разные источники под разные вопросы.** Часть вопросов — в базу знаний, часть — в SQL по таблице, часть —
  в свежий веб. Фиксированный пайплайн ходит всегда в одно место. Агент **маршрутизирует** запрос туда, где
  лежит ответ.
- **Плохой первый результат.** Достали нерелевантные чанки — статический конвейер всё равно отдаёт их в
  ...
  сходить снова. Это **самокоррекция**.
...
```text
    ┌──────────────────────────────────────────────┐
    ▼                                              │
[ Рассуждение ] → [ Решение: ] → [ Действие ] → [ Наблюдение ]
  что знаю,        какое           вызов           результат
  чего нет         действие        ретрива/        вернулся
                                   инструмента      в контекст
    │                                                │
    └── достаточно? → да → [ Ответ ] ◄───────────────┘
```
...
внутри цикла**, а не первая ступень фиксированной лестницы.
...
Разложим «модель управляет» на осязаемые способности.
...
Не думай «статический ИЛИ агентный». Между ними — плавный спектр по тому, **сколько свободы отдано модели**.

1. **Router (маршрутизатор).** Самый лёгкий шаг в агентность. Модель делает один выбор — куда направить
   запрос (в какой индекс, в какой инструмент, или «retrieval не нужен») — а дальше всё статично. Дёшево,
   предсказуемо, покрывает большую долю пользы.
2. **Query planning.** Модель заранее раскладывает сложный вопрос на подзапросы.
3. **Полный цикл (ReAct-подобный).** Полноценный `рассуждение → действие → наблюдение` в цикле, с
...
не приз, а расход. Часто router поверх хорошего статического RAG бьёт «полного агента» по стоимости,
латентности и стабильности.

## Расплата — и мост назад к Части I

Отдав управление модели, ты платишь ровно за это.

- **Латентность и стоимость.** N шагов — это N вызовов LLM плюс N ретривов. Один вопрос легко превращается в
...
- Агентность нужна там, где конвейер ломается: **multi-hop**, «retrieval не нужен», **маршрутизация по
...
- Это **спектр**: router → query planning → полный цикл. Бери **самый простой уровень**, который решает
  задачу.
...
**Новые термины** → [Глоссарий](../glossary.md): agentic RAG, agent loop, reasoning–action–observation
(ReAct), routing / query router, multi-hop retrieval, query planning, self-correction / self-reflection,
iterative retrieval.

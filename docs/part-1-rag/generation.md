...
контекста**. Вернись к рамке из первого урока: **generation failure** — это когда нужный чанк *был* в
...
- **Явное разделение контекста.** Помечай, где начинаются и заканчиваются источники, чтобы модель отличала
  «данные для ответа» от «инструкций». Это заодно первая линия обороны против prompt injection (об этом — в
  слое Guardrails).
- **Порядок: эффект lost-in-the-middle.** Модель лучше «видит» начало и конец длинного контекста, а
  закопанное в середине теряет. Отсюда правило: не вываливать 50 чанков, а подать несколько лучших (спасибо
  реранкингу) и поставить самые релевантные по краям.
...
контексту. Если ответа в нём нет — так и скажи, не выдумывай». Одна эта инструкция убирает бóльшую часть
галлюцинаций, потому что снимает у модели «право» дополнять ответ из памяти.
...
Система должна иметь право сказать «не знаю» — и должна быть на это проинструктирована. Уверенный
неправильный ответ хуже честного «в документах этого нет». Если retrieval ничего не нашёл, это должно выйти
**отказом**, а не выдуманным ответом. В корпоративной среде это принципиально: цена ложного факта, поданного
уверенно, выше цены пустого ответа.
...
**измеряют** метрикой faithfulness / groundedness. Формализуем её в слое [Evaluation](./cross-cutting.md);
пока держи в голове, что «модель ведёт себя хорошо» — это не ощущение, а число.
...
- **Цитирование** даёт проверяемость и само по себе снижает выдумки.
- **Отказ** — штатное поведение, а не сбой.

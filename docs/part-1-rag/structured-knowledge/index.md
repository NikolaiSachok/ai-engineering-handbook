...
why it deserved three lessons. But three ordinary questions break it, and they break it in three different
places:
...
mature answer that is not a vector index. This lesson is about the three of them, and mostly about **when
they are worth building**, because all three are heavily marketed and all three are frequently the wrong
call.

:::note[Read the section you need]

The three parts stand alone and there is no reading order between them. If you are deciding whether to
extract structure at all, start with [the vocabulary ladder](#three-kinds-of-structure). If someone has
asked for a knowledge graph, go to [when a graph earns its build cost](#when-a-graph-earns-its-build-cost).
...
Three things get called by each other's names constantly, and separating them is most of what this section
is worth.

| | What it is | What it buys you |
|---|---|---|
| **Controlled vocabulary** | a fixed list of allowed terms | the extractor stops inventing labels |
...
**Most teams that say they need an ontology need a controlled vocabulary.** That is the single most useful
sentence here, and it is worth saying to a stakeholder early, because the gap between those two artefacts is
roughly the gap between an afternoon and a permanent staffing commitment.

Here is the failure a controlled vocabulary fixes, and it is the most common structured-extraction failure
there is. You ask a model to pull relationships out of three thousand documents. It does, fluently. In one
document a person `works_for` a company; in the next they are `employed_by` it; in the third they are an
...
them. Hand it a closed list of permitted relation types and the problem disappears, and it disappears for
the cost of writing the list.

### Where a schema actually bites

Schema discipline pays in three separable places, and it is worth knowing which one you are buying:
...
argues for in [layered gates and mechanism diversity](/ai-sdlc/part-3-verification/layered-gates). The
argument is made there; what matters here is that a schema is what makes such a gate *possible*, because a
gate needs something to check against.
...
### The conflation worth naming: domain schema is not response schema
...
Start from what you can already do, because the usual opening claim for knowledge graphs — that they solve
multi-hop questions — is answered better and more cheaply elsewhere in this course.

*"Who leads the team that shipped policy X?"* looks like it needs a graph traversal. It doesn't. Decompose
it into two ordinary retrievals — find the team that shipped X, then find who leads that team — and the
static pipeline you already have answers it. That is [agentic RAG](../../part-2-agents/agentic-rag/index.md),
...
The reason exists, but it is a different question class.
...
with the supplier. The honest position is that the win is **modest**, and it is not what justifies the
machinery.

**Global — the whole corpus.** *"What themes recur across these ten thousand documents?"* *"Which risks
appear in more than one contract family?"* **No retrieval over chunks can answer this**, and the reason is
worth being precise about: retrieval assumes the answer exists in the corpus and your job is to locate it.
Here there is no chunk to locate. The answer is a *summary of the corpus*, and it has to be constructed
before anyone asks. This is the case that justifies the build, and it is the case the GraphRAG paper is
about — conventional RAG, its authors argue, "fails on global questions directed at an entire text corpus"
...
`ACME Corp.`, and a misspelling. A graph is where this problem becomes visible — and, as the deep dive
covers, it is also where graph work quietly turns into [entity-resolution work](./deep-dive.md#entity-resolution-is-the-unsolved-middle),
which is a different discipline with a different budget.
...
was built specifically to avoid "prohibitive up-front indexing costs", and its indexing is reported as
...
put in a proposal, and it is the vendor's own.

**The extraction hallucinates edges.** A relation the model inferred but no document states will sit in the
graph looking exactly like one that was — incorrect facts in a corpus yield erroneous triples in an
...
decides it in practice: **nobody has committed to keeping the extraction schema correct.** Without that
owner, the graph is a snapshot degrading from day one, and the failure will be blamed on the technology
rather than on the staffing decision that caused it.
...
**(a) The metrics layer.** A modelling tier over a warehouse — dbt's Semantic Layer, Cube, LookML and their
...
**(b) The semantic layer over the linguistic layer.** In conversational AI: the assistant resolves an
...
The practical payload belongs to meaning (a), and it is the clearest example in this lesson of a structural
change to a problem's difficulty.
...
source system left them in. On [BIRD](https://bird-bench.github.io/) — a benchmark built specifically around
large, realistic, messy databases, where values "retain their original and frequently 'dirty' format" —
human data engineers reach **92.96%** execution accuracy while the leading system reaches **81.95%** (both
as reported on the leaderboard, September 2025). Roughly one query in five is wrong on realistic schemas,
...
detectable — you can show which metric was chosen — where a subtly wrong join is not.
...
There is a second thing you get, and it matters more than it sounds. Because every query passes through the
layer, **access policies apply deterministically before the warehouse is touched** — Cube's phrasing is that
a query "is validated against the data model and has access policies applied deterministically before
reaching the warehouse". That is the same
principle the retrieval lesson insists on — [cut before the search, never
post-filter](../retrieval/index.md) — arriving from the structured side.

### Why enterprises want meaning (b)
...
This is also where the two halves of this lesson meet, because the domain model being resolved against is
exactly [the ontology](#three-kinds-of-structure) — which is why "we need an ontology" and "we need a
...
so someone owns them permanently — and a metric that silently changes meaning is worse than no metric, for
the same reason a wrong ontology is worse than none.
...
## Which of the three you actually need \{#which-of-the-three-you-actually-need}

The three sections answer one question in three places: *where does the structure in your knowledge live,
...
- If your questions are about the corpus as a whole rather than about anything in it, you need a
  **graph** — and an owner for its schema, or you do not.
...
- If none of those describe you, the static pipeline in the first three lessons is the right architecture
  and the structured detour is a cost with no return.

What unites the three failure modes is worth carrying: each of these artefacts fails **silently and
plausibly**. A wrong ontology validates. A stale graph returns confident relations. A drifted metric renders
in a dashboard. None of them throws. Every one of them needs a named owner and a check that would catch it,
which is why each section here ends on maintenance rather than on capability.
...
- Controlled vocabulary, taxonomy and ontology are three different commitments — most requests for the third
  are satisfied by the first, and the test for going further is naming a query you cannot answer today.
...
- The graph build is an LLM pass over the whole corpus, it can hallucinate edges, and it goes stale — with
  the practical decider being whether anyone owns the extraction schema.
- "Semantic layer" names both a metrics tier over a warehouse and concept resolution in conversation; say
  which you mean first.
- Against a semantic layer a model selects a defined metric instead of deriving a query, which shrinks the
  decision and changes the failure into a visible one — and lets access policies apply before the warehouse
  is touched.
- Each artefact here fails silently and plausibly, so each needs an owner rather than a launch.

**[New terms](../../glossary.md#structured-knowledge)**: controlled vocabulary, taxonomy, ontology, RDF, OWL,
SHACL, SPARQL, knowledge graph, GraphRAG, entity resolution, community detection, semantic layer, metrics
layer, text-to-SQL.
...
**[Extraction, schemas and the query surface](./deep-dive.md)** — how a graph is actually built (the six
...
See also: where chunks come from — [Ingestion](../ingestion/index.md); the routing machinery this lesson
reuses — [Retrieval](../retrieval/deep-dive.md); multi-hop without a graph —

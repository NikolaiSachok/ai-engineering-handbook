...
Routing also has a destination this lesson doesn't build. "Which index" quietly assumes every branch ends in a vector search, and some questions need a structured path instead — an aggregate that a top-K physically cannot compute, or a corpus-level question that no single chunk answers. Where those branches lead, and when they're worth constructing at all, is [structured knowledge](../structured-knowledge/index.md); the router here is the mechanism that sends a query down one of them.
...
**[New terms](../../glossary.md#retrieval)**: score fusion / score normalisation, LLM reranker, late interaction / ColBERT, multi-vector retrieval, contextual retrieval, query routing, pre-filter / post-filter.

...
Routing also has destinations this lesson doesn't build. "Which index" quietly assumes every branch ends in a vector search, and some questions need a structured path instead — an aggregate that a top-K physically cannot compute, or a corpus-level question that no single chunk answers. Where those branches lead, and when they're worth constructing at all, is the subject of [structured knowledge](../structured-knowledge/index.md); the router is what sends a query down one of them.

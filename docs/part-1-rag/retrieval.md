...
то, как из «ближайших векторов» получить выдачу, которая действительно релевантна, правильно упорядочена
и разрешена конкретному пользователю.

Держи рамку из первого урока: **retrieval failure** — это когда нужного чанка нет в том, что мы вернули.
...
чанк мгновенно. Добавь сюда разрыв в формулировках (пользователь спросил не теми словами, что в документе)
и полное отсутствие понятия прав доступа — и становится ясно, что поверх наивного поиска нужны надстройки.
...
LLM-вызовов *перед* retrieval заметно поднимают попадание.
...
Самый весомый апгрейд над наивным поиском. Работают два механизма с противоположными сильными сторонами:

| | **Dense (векторный)** | **Sparse / keyword (BM25)** |
|---|---|---|
...
Гибрид запускает оба и объединяет их оценки (взвешенно или через Reciprocal Rank Fusion). Каждый закрывает
...
топ-K, где K — это 50–100. Но порядок внутри этой сотни грубый, а в контекст модели влезает лишь несколько
чанков. Поэтому вторая стадия работает на **точность**: cross-encoder из [прошлого урока](./ingestion.md)
переоценивает каждого кандидата против запроса и пересортировывает список, поднимая лучшее наверх. В
генерацию уходят только верхние несколько.

Это каноническая двухстадийная схема: дёшево и широко (bi-encoder или гибрид — полнота), затем дорого и
точно (cross-encoder — точность). Здесь bi- и cross-encoder из Урока 2 складываются в единую картину.

## Фильтры и access control — релевантность и права
...
- **Access control.** Права отсекают *до* выдачи, чтобы пользователь физически не мог получить чанк, к
  которому у него нет доступа (пример с зарплатной ведомостью из первого урока). Это жёсткое требование:
...
запрос → [transform] → [гибрид: dense + BM25, фильтр метаданных + ACL]
       → кандидаты (топ-K) → [реранк: cross-encoder] → топ-несколько → в генерацию
```
...
- Наивный векторный top-K — старт, а не финал.
- Query transformation чинит сам запрос: переписывание, multi-query, HyDE.
- Гибридный поиск (dense + BM25) закрывает разрыв между смыслом и точным словом — ключевой апгрейд.
- Реранкинг (cross-encoder) чинит порядок: стадия полноты, затем стадия точности.
- Фильтры и access control дают релевантность вместе с правами; ACL — это требование безопасности.

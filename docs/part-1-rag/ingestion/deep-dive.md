...
- **Fine-tuning is contrastive learning on triples from your domain** (a query, a right passage, a wrong one) — powered by hard negatives, bootstrappable with synthetic queries, and a committing decision because it forces a full re-index; reach for a better off-the-shelf model and hybrid search first.
...
**[New terms](../../glossary.md#ingestion-chunking)**: document parsing / layout-aware extraction, OCR, late chunking, embedding fine-tuning, Matryoshka Representation Learning (MRL), contextual retrieval, multilingual embeddings, dimensionality.

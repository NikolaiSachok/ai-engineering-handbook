...
Оно написано сразу для трёх аудиторий:

- **Изучающие тему** — любой инженер, который хочет разобраться в RAG и агентах всерьёз: с разбором «почему»
  и режимов отказа, а не только перечнем возможностей.
- **Справочник автора** — долговременная запись принципов проектирования и принятых решений.
- **Портфолио** — демонстрация зрелой инженерной практики: eval, guardrails, observability, дисциплина
  проектирования.

Мы предполагаем опытного инженера-читателя: коммодити-инструменты (векторные БД, оркестраторы) не разбираем
...
Каждая часть открывается страницей-обзором: с чего она начинается, что внутри и что нужно знать заранее.

...
Prečo na tom pri agentoch záleží? Opäť pre ten rozdiel, ktorý vnáša AI. Behy agentov sú dlhé, nedeterministické a drahé — tridsaťkrokový výskumný agent, ktorý zomrie na kroku 28, nesmie znova zaplatiť za 27 volaní modelu, aby sa dostal späť. Vďaka odolnosti sa stáva reálnym aj human-in-the-loop: uzol-prerušenie uloží stav a beh jednoducho čaká hodiny aj dni, kým to niekto schváli; pretrvá aj po reštarte či novom nasadení. Vykonávanie však nepokračuje tam, kde sa uzol zastavil. Keď to človek schváli, framework spustí celý prerušený uzol nanovo od prvého riadka, takže každý riadok pred volaním `interrupt()` sa vykoná druhý raz. Ani jeden režim `durability` na tom nič nemení — režimy rozhodujú o tom, kedy sa stav zapíše, nie o tom, odkiaľ sa vykonávanie po pauze obnoví. Preto v uzle, ktorý sa preruší, nevykonávaj spoplatnené ani nevratné operácie. Platbu presuň do samostatného uzla pred pauzou, alebo ju sprav idempotentnou pomocou kľúča, ktorý sa pri opätovnom vykonaní uzla nezmení. Je to tá istá [zásada kľúča idempotencie](../tool-use/deep-dive.md) ako pri používaní nástrojov, len uplatnená vo vlastnom grafe. To je HITL uzol z prvej časti, ukotvený v tej istej perzistencii. Všeobecná vrstva human-in-the-loop a rozpočtu je [plánovanie a slučky](../planning-loops/index.md); tu je to len uzol podopretý checkpointom.
...
**[Nové pojmy](../../glossary.md#orchestration-frameworks)**: state graph, checkpointer, checkpoint backend, thread (thread_id), durable execution, conditional edge, framework long-term memory (store), declarative vs imperative agent definition.

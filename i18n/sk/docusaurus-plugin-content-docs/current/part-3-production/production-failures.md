...
Testovacie prípady napísané v prvom týždni opisujú, ako si tím predstavoval, že sa ľudia budú pýtať. Požiadavky zozbierané za šesť mesiacov produkčnej prevádzky — teda to, čo do systému skutočne prišlo od používateľov, aj s odpoveďou, ktorú na to dostali — opisujú, ako sa pýtajú naozaj, a v tej medzere začína zelený panel klamať. Bežné odporúčanie žiada každý týždeň vzorkovať požiadavky z produkcie a používať tú vzorku ako benchmark (meradlo, voči ktorému sa porovnávaš). Tu je naša prvá nezhoda: *nahradiť* stálu sadu znamená vymeniť jedno slepé miesto za druhé. Benchmark, ktorý sa mení každý týždeň, ti nepovie, či zmena z tohto týždňa pokazila niečo, čo minulý týždeň fungovalo; presne na to je zmrazená sada.

Ponechaj si obe. **Zmrazená regresná sada** odpovedá na otázku „pokazil som niečo, čo fungovalo?“ — a aby odpovedala, musí zostať nemenná. **Rotujúca sada vzorkovaná z produkcie** odpovedá na otázku „zodpovedá moja evaluácia ešte realite?“ — a aby odpovedala, musí sa hýbať. Na rotáciu je aj druhý dôvod a prichádza z opačnej strany: pevný benchmark, podľa ktorého tím mesiace optimalizuje, prestane merať kvalitu a začne merať, ako dobre ten benchmark pozná. Obe sady potrebujú značky, a práve s tým nikto v rozpočte nepočíta — [evaluácia](../part-1-rag/cross-cutting/evaluation/index.md) to hovorí bez okolkov: bez dátovej sady niet evaluácie.
...
Medzeru zaplnia dve veci. Prvá je **trace** — záznam behu, ktorý zachytí celú cestu jednej požiadavky: dopyt, vrátené chunky aj ich skóre, odoslaný prompt, odpoveď a tokeny. Bez identifikátorov chunkov totiž nezrekonštruuješ ani to, *prečo* bola odpoveď nesprávna. Druhá je **nezávislý sudca nad vzorkou požiadaviek z produkcie** (LLM-as-a-judge), aby sa kvalita stala monitorovanou metrikou s prahom a s upozornením, nie niečím, čo sa dozvieš až z hlásenia na podpore. To je [Observability (pozorovateľnosť)](../part-1-rag/cross-cutting/observability/index.md) — aj tá slučka, ktorou sa jej zistenia vracajú do evaluácie.
...
- Potrebuješ dve evaluačné sady: zmrazená odhaľuje regresie, rotujúca sada vzorkovaná z produkcie udržiava evaluáciu blízko skutočnosti; ani jedna nenahrádza druhú.
- Dostupnosť nie je správnosť. Pridaj trace s identifikátormi chunkov a sudcu nad vzorkou požiadaviek z produkcie; a samostatne rozhodni, či máš voči niekomu povinnosť viesť audítorskú stopu.

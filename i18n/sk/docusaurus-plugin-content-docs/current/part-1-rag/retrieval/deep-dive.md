...
Router môže smerovať aj do systémov, ktoré táto lekcia nestavia. „V ktorom indexe“ potichu predpokladá, že každá vetva končí vektorovým vyhľadávaním — a niektoré otázky namiesto toho potrebujú štruktúrovanú cestu — napríklad agregačná otázka, ktorú z výsledkov top-K vypočítať nemožno, alebo otázka o celom korpuse, na ktorú neodpovie žiadny jednotlivý chunk. Kam tie vetvy vedú a kedy sa ich vôbec oplatí stavať, rozoberajú [štruktúrované znalosti](../structured-knowledge/index.md); router je to, čo dopyt pošle jednou z nich.
...
**[Nové pojmy](../../glossary.md#retrieval)**: score fusion / score normalisation, LLM reranker, late interaction / ColBERT, multi-vector retrieval, contextual retrieval, query routing, pre-filter / post-filter.

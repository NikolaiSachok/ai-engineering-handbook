...
Одну ступень этой лестницы книга до сих пор только перечисляла, а продают её все платформы ниже.
**Дистилляция** учит маленькую **модель-ученика** повторять выходы большой **модели-учителя** — ты не сжимаешь
учителя, ты обучаешь другую, меньшую модель повторять поведение учителя на том распределении данных, с которым
ученику предстоит работать.
Экономия здесь берётся оттого, что учителя достаточно один раз прогнать по твоим обучающим данным, а
обслуживаешь ты дальше только ученика. Обратная сторона: ученик — это *новая модель*, которую надо отдельно
оценивать, проверять на регрессии и следить за её дрейфом. Это легко недооценить, и цена ошибки высока: ученик хорош ровно
настолько, насколько его обучающая выборка отражает реальный трафик, — значит, он совпадёт с учителем на тех участках
распределения, которые попали в выборку, и тихо разойдётся с ним на остальных. Учи ученика на продовом трафике, а не на
вылизанном наборе, и держи учителя под рукой как точку сравнения.

Есть и родственный приём, решающий другую задачу. **Дистилляция контекста (context distillation)** учит
модель вести себя так, будто ей по-прежнему передают длинный контекст: системный промпт, политику, набор
разобранных примеров, — хотя на инференсе его уже не подают. Модель обучают так, чтобы без контекста она давала
примерно тот же ответ, что и с ним ([Snell, Klein и Zhong](https://arxiv.org/abs/2209.15189);
Askell и др. этим приёмом встроили в веса промпт, задающий безопасное поведение модели). Место ей на этой
лестнице, а не рядом с квантизацией, потому что она работает с *контекстом*: она отвечает на вопрос «как
перестать платить за эти четыре тысячи токенов промпта в каждом запросе». Плата та же, что и за остальные способы
адаптации: поведение теперь закреплено в весах, поэтому смена политики требует нового обучения, а не правки
строки. Кэширование промпта тоже сокращает расходы на повторяющийся контекст, но такой зависимости не
создаёт — с него и надо начинать.
...
[уроке про экосистему инструментов](../tooling-ecosystem/index.md).
...
запросов между моделями ради цены — живёт в [уроке про LLMOps](../llmops/index.md).
...
LiteLLM и шлюзы разбирает [урок про LLMOps](../llmops/index.md); здесь мы их не переписываем.
...
**[Новые термины](../../glossary.md#cloud-platforms)**: fine-tuning, LoRA / PEFT, DPO, reinforcement fine-tuning
(RFT), model distillation, context distillation, continued pre-training, managed agent runtime, FinOps, cost modelling, unit

...
# Где считаются твои токены

[Урок про сервинг](../serving/index.md) закончился развилкой: нижний блок архитектуры — инференс-сервер
...
```mermaid
flowchart TD
    APP["Твоё приложение"] --> OWN["Свои GPU<br/>+ инференс-сервер"]
    APP --> VENDOR["Прямой API<br/>поставщика модели"]
    APP --> CLOUD["Управляемая<br/>AI-платформа облака"]
    OWN --> OWN2(["Максимум контроля,<br/>вся эксплуатация твоя"])
    VENDOR --> VENDOR2(["Максимум простоты,<br/>данные уходят,<br/>отдельный контракт"])
    CLOUD --> CLOUD2(["Внутри периметра облака:<br/>IAM, биллинг, аудит"])
```
...
[уроке про MCP](../../part-2-agents/mcp/index.md) мы уже договаривались держаться разделительных черт — границ
...
[уроке про ingestion](../../part-1-rag/ingestion/index.md) мы выбирали между self-hosted эмбеддингами и
...
корпуса, собственный конвейер — когда [оценка](../../part-1-rag/cross-cutting/evaluation/index.md) показывает, что
...
[урока про guardrails](../../part-1-rag/cross-cutting/guardrails/index.md), реализованные как управляемые сервисы;
...
Vertex/GEAP). Не путай его с непрерывным батчингом из [урока про сервинг](../serving/index.md): тот живёт в
...
никуда не девается, а в [уроке про LLMOps](../llmops/index.md) мы достроим её: добавим маршрутизацию запросов
...
[уроке про LLMOps](../llmops/index.md) посмотрим на [LiteLLM](https://www.litellm.ai) и шлюзы подробнее). Сильнее всего привязывают
...
**[Новые термины](../../glossary.md#cloud-platforms)**: managed endpoint, model catalog, data residency,
...
**[Стоимость, агенты, суверенитет](./deep-dive.md)** — мастерский проход по тем же платформам: дообучение
(fine-tuning) на платформах и где оно уместно, управляемые агентные среды (Bedrock AgentCore, Foundry Agent
Service, Vertex Agent Engine), моделирование стоимости и FinOps (форма тарифов у каждой платформы, скидки за
обязательство по объёму, плата за межрегиональный трафик), мультиоблачные шлюзы и суверенные облака.

См. также, в Части III: [сервинг](../serving/index.md) — развилка «арендовать или держать своё», на которую
отвечает этот урок; [LLMOps](../llmops/index.md) — управление расходами на уровне организации и маршрутизация между
моделями; [экосистема инструментов](../tooling-ecosystem/index.md) — «строить или брать» вокруг платформы. А
[углубление урока про сервинг](../serving/deep-dive.md) разбирает serverless-GPU и экономику «всегда тёплый
GPU против масштабирования в ноль» со стороны self-host.

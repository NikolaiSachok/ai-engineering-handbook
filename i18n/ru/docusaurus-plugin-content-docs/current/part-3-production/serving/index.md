...
работает сама модель — про это урок об [облачных AI-платформах](../cloud-platforms/index.md), — потом посмотрим,
[чем обвязать систему в проде](../tooling-ecosystem/index.md), и закончим тем, [как с ней жить после
релиза](../llmops/index.md).
...
sequenceDiagram
    ...
    participant S as Сервис приложения
    participant R as Retrieval
    participant M as LLM
    U->>S: POST /ask
    S->>R: Поиск по базе знаний
    R-->>S: Выдача (чанки)
    S->>M: Генерация (stream: true)
    M-->>S: Токены (SSE)
    S-->>U: SSE-события, токен за токеном
    Note over U,S: Статус 200 уже ушёл —<br/>сбой передаётся внутри потока
```
...
разговор в уроке про [экосистему инструментов](../tooling-ecosystem/index.md).
...
flowchart TB
    ...
    A -->|Своё железо| V["Инференс-сервер (vLLM / SGLang)<br/>Непрерывный батчинг, KV-кэш"]
    V --> G[GPU]
```
...
**[Новые термины](../../glossary.md#serving)**: serving, inference, inference server, SSE (Server-Sent
...
См. также, в Части III: [облачные AI-платформы](../cloud-platforms/index.md) — арендовать или держать своё и где
работает модель; [LLMOps](../llmops/index.md) — как жить с сервисом после релиза; [экосистема
инструментов](../tooling-ecosystem/index.md) — чем его обвязать. А про SLO и бюджеты задержки, ради которых всё

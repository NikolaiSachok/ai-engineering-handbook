...
flowchart TB
    R["Запросы"] --> SEM
    subgraph AL["Прикладной слой"]
        SEM["Семафор<br/>предел параллелизма N"] --> Q{"Очередь<br/>глубина D"}
        Q -->|"Очередь заполнена"| SHED["Сброс нагрузки<br/>429 / 503<br/>+ Retry-After"]
    end
    Q -->|"Допущен"| SCHED
    subgraph IS["Инференс-сервер"]
        SCHED["Контроль допуска<br/>max_num_seqs"] --> GPU["GPU<br/>пул KV-кэша"]
    end
```
...
Квантизацию путают с двумя другими операциями, потому что все три проходят под шапкой «сделать модель дешевле»,
а две из трёх ещё и связаны с обучением. Различает их то, что именно меняет каждая:

| Операция | Что меняет | Есть ли обучение | На каком этапе решают |
|---|---|---|---|
| **Квантизация** | *представление* чисел — меньше битов на один вес или активацию, модель та же | нет, если квантизацию делают после обучения, — в сервинге применяют именно такую (обучение с учётом квантизации существует, но это выбор на стадии адаптации) | сервинг |
| **Дистилляция** | создаёт *новую, меньшую модель*, обученную повторять выходы большой модели-учителя | да — новые веса и новая модель, которую надо оценивать | адаптация |
| **Дообучение** | сами веса, чтобы модель вела себя иначе | да — на твоих данных | адаптация |

К сервингу из трёх относится только первая, поэтому она и живёт на этой странице; две другие стоят на
[лестнице адаптации](../cloud-platforms/deep-dive.md). Есть и ещё одно, совсем другое употребление слова
*дистилляция*: **дистилляцией находки** в
[углублении про agentic RAG](../../part-2-agents/agentic-rag/deep-dive.md) называют сжатие текста, который
передают между шагами поиска внутри одного запроса. Ни обучения, ни новых весов — то же слово на совсем
другой работе.
...
```mermaid
flowchart LR
    subgraph TP["Одна машина — тензорный параллелизм"]
        direction LR
        G0["GPU0<br/>половина матрицы слоя"] <-->|NVLink| G1["GPU1<br/>вторая половина"]
    end
    subgraph PP["Две машины — конвейерный параллелизм"]
        direction LR
        NA["Машина A<br/>слои 0…N"] -->|Медленная сеть — микробатчи| NB["Машина B<br/>слои N+1…конец"]
    end
```
...
- Квантизацию, дистилляцию и дообучение складывают в одну кучу «сделать дешевле», а взаимозаменяемыми они не
  являются: на этапе сервинга выбирают только квантизацию — она меняет то, как представлены числа, уже после
  обучения; две другие порождают новые веса, которые надо оценивать, и стоят на лестнице адаптации.
...
**[Новые термины](../../glossary.md#serving)**: ASGI-воркеры, uvloop, пул потоков, backpressure, load shedding, контроль допуска, закон Литтла, планирование на уровне итерации, prefill / decode, chunked prefill, кэширование префикса, квантизация, квантизация KV-кэша, тензорный параллелизм, конвейерный параллелизм, параллелизм данных, MIG (Multi-Instance GPU), нарезка GPU по времени, KEDA, KServe, serverless-GPU.

...
Раз уронить качество может любое изменение, у каждого изменения должна быть регрессионная проверка
(quality gate). Ею
...
```mermaid
flowchart TB
    C["Изменение<br/>промпт / модель / индекс / конфиг"] --> E["Eval в CI<br/>golden set"]
    E -- "балл ниже порога" --> C
    E -- "порог пройден" --> D["Деплой<br/>канареечный / теневой"]
    D --> P["Прод<br/>трейсы, метрики, стоимость"]
    P --> T["Плохие трейсы<br/>новые eval-кейсы"]
    T --> C
```
...
этот путь буквально кнопкой «превратить трейс в eval-кейс». Так каждый инцидент ужесточает регрессионную
проверку: однажды пойманная беда второй раз в прод не пройдёт.
...
до прода. От первого чанка до системы, которая работает под нагрузкой, проходит регрессионные проверки и
укладывается в бюджет, — вот и весь путь. База пройдена; дальше — углублённые проходы по слоям.
...
- **Артефакт LLM-приложения** — не только код: промпты, версия модели, снимок индекса, конфигурация
  ...
  пустом диффе кода.
- **Eval в CI** — регрессионная проверка: на каждое изменение гоняется golden set, метрики ниже порога
  блокируют слияние.
...
**[Новые термины](../../glossary.md#llmops)**: LLMOps, canary release, shadow deployment, prompt
...
**[Дообучение, расходы, очереди](./deep-dive.md)** — глубокий второй проход: операционная сторона дообучения (когда дообучать вместо правки промпта и как откатить дообученную модель), управление расходами на уровне организации, релизная проверка и откат как взгляд на разбор регрессий, бюджеты ошибок как организационный процесс и инфраструктура очередей для пакетных работ.

См. также: [сервинг](../serving/index.md), [облачные платформы](../cloud-platforms/index.md), [экосистема инструментов](../tooling-ecosystem/index.md) и [углублённый разбор наблюдаемости](../../part-1-rag/cross-cutting/observability/deep-dive.md).

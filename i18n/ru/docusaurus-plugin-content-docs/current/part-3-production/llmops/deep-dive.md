...
политики ограничителей»; eval в CI служит механизмом контроля, за которым изменение либо проходит, либо блокируется;
...
организации, взгляд «релизная проверка и откат» на регрессии, бюджет ошибок как подписанную политику и
...
прогони дообученную модель через ту же регрессионную проверку, что и любой деплой (тот самый eval в CI из
...
flowchart TD
    ...
    K -->|нет, это поведение/формат/тон/навык| T["Дообучение: собрать данные →<br/>отложить eval → обучить →<br/>регрессионная проверка → зафиксировать"]
    T --> R["Регрессия? Вернуть прежнюю версию /<br/>отцепить LoRA-адаптер"]
```

В реальной организации до вопроса «знание или поведение» дело часто не доходит: ответ определяют два других
критерия. Оба уже разбирались в других уроках, но там не было видно, насколько прямо они влияют на этот
выбор.

Права каждого пользователя проверяются фильтром поиска — в замороженных весах им места нет.
[Углубление слоя поиска](../../part-1-rag/retrieval/deep-dive.md) описывает контроль доступа как часть механики
поиска — отсекать до поиска, а не фильтровать после, — поэтому он и читается как деталь того, как исполняется
запрос. Но архитектурное следствие шире: фильтр может быть разным для каждого пользователя, а веса у всех одни и те
же. Что бы дообученная модель ни выучила, это достанется каждому, кто её вызовет. Значит, в
организации, где доступ к собственным знаниям у разных людей разный, этот критерий определяет архитектуру сам
по себе, ещё до всякого расчёта стоимости. В весах закрепляют поведение и формат; знание остаётся в
хранилище, за фильтром, потому что только фильтр умеет учесть права конкретного пользователя.

Ещё одна ветка, которой нет на схеме выше, — сравнение размера корпуса с окном контекста. [Углубление слоя
генерации](../../part-1-rag/generation/deep-dive.md) разбирает длинный контекст подробно, но как задачу
*упаковки* внутри RAG: lost-in-the-middle, полезный контекст не равен размеру окна. Отсюда встаёт более
ранний вопрос: если весь корпус влезает и остаётся маленьким, отдельный слой поиска, возможно, вообще не
нужен, дообучение тем более нечем оправдать, а всё, что ты добавишь сверху, останется без работы. В том
углублении уже сформулирована оговорка, и здесь она не повторяется: *влезть — ещё не значит быть прочитанным*. Так что
«влезает» — это место, где проверка начинается, а не заканчивается. Измерь качество поиска на своих вопросах,
прежде чем удалять слой поиска.

Ни один из двух критериев не заменяет довод о свежести выше; оба надо проверить раньше. Свежесть говорит, *могут* ли
факты жить в весах. Эти два говорят, *позволено* ли им там жить и нужна ли тебе вся эта механика вообще.
...
- **Разбор стоимости в чек-листе деплоя** — раз изменение промпта есть изменение стоимости, релизная проверка
  (следующий раздел) оценивает не только качество, но и прогноз стоимости на запрос.
...
Eval в CI из Части 1, если смотреть на него со стороны релиза, — это **блокирующая релизная проверка**
качества: изменение, чьи метрики на golden set упали ниже порога, — это регрессия, пойманная до выкатки, и
слияние или деплой блокируются. Здесь — самое дешёвое место остановить регрессию: на входе, а не в проде.
...
Сильнее любой проверки на отдельном изменении — только одно: когда организация говорит «сейчас никаких
релизов вообще». Такое решение — **заморозка релизов**, и правит ею политика бюджета ошибок из следующего
раздела.

```mermaid
flowchart TB
    C["Изменение"] --> EG{"Релизная проверка<br/>(golden set)"}
...
- Дообучай последним — после того как промпт и RAG упёрлись в потолок — и только под поведение, формат, тон
  или навык, никогда ради свежего знания (это работа RAG). Дообученная модель — зафиксированный артефакт, проходящий
  ту же регрессионную проверку, а откат её — возврат прежней версии или отцепление LoRA-адаптера.
- До вопроса «знание или поведение» идут два критерия: различия в правах пользователей учитывает фильтр поиска,
  а общие для всех веса — нет, поэтому разграниченный доступ определяет архитектуру сам по себе; а если корпус влезает
  в окно и остаётся маленьким, то, возможно, искать и дообучать вообще нечего — но влезть ещё не значит быть
  прочитанным.
...
- Релизная сторона регрессии: релизная проверка блокирует её до выкатки; постепенная раскатка делает автооткат
...
**[Новые термины](../../glossary.md#llmops)**: SFT, DPO, RFT, grader, LoRA / PEFT, showback, chargeback,

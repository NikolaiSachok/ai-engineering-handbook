...
организации, взгляд «релизный контроль и откат» на регрессии, бюджет ошибок как подписанную политику и
...
прогони дообученную модель через тот же регрессионный контроль, что и любой деплой (тот самый eval в CI из
...
flowchart TD
    ...
    K -->|нет, это поведение/формат/тон/навык| T["Дообучение: собрать данные →<br/>отложить eval → обучить →<br/>регрессионный контроль → зафиксировать"]
...
- **Разбор стоимости в чек-листе деплоя** — раз изменение промпта есть изменение стоимости, релизный контроль
  (следующий раздел) проверяет не только качество, но и прогноз стоимости на запрос.
...
Eval в CI из Части 1, если смотреть на него со стороны релиза, — это **релизный контроль** качества:
изменение, чьи метрики на golden set упали ниже порога, — это регрессия, пойманная до выкатки, и
слияние или деплой блокируются. Такой контроль — самое дешёвое место остановить регрессию: на входе, а не в проде.
...
```mermaid
flowchart LR
    C["Изменение"] --> EG{"Релизный контроль<br/>(golden set)"}
...
- Дообучай последним — после того как промпт и RAG упёрлись в потолок — и только под поведение, формат, тон
  ...
  тот же регрессионный контроль, а откат её — возврат прежней версии или отцепление LoRA-адаптера.
...
- Релизная сторона регрессии: релизный контроль блокирует её до выкатки; постепенная раскатка делает автооткат

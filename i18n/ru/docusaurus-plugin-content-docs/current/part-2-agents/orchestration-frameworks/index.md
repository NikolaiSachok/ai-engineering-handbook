...
- Узел-прерывание для режима **человек-в-цикле** (human-in-the-loop, HITL): цикл замирает, ждёт
  человеческого одобрения, а потом узел прогоняется заново, с первой строки. Это тот самый человек-в-цикле
  из урока про планирование — теперь оформленный как полноправный узел графа, а не как ручная остановка сбоку.
...
**[Новые термины](../../glossary.md#orchestration-frameworks)**: orchestration framework, agent as a graph / state machine, node / edge, checkpointing, human-in-the-loop (HITL).

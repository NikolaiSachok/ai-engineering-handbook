...
и прогон часами или днями ждёт одобрения, переживая перезапуски и деплои. Подхватить работу с середины узла
нельзя. После одобрения узел исполняется ещё раз, с самой первой строки. Весь код до вызова `interrupt()`
отрабатывает второй раз. Настройка `durability` тут ничего не меняет: её режимы определяют момент записи
состояния, а не точку, с которой начнётся исполнение. Отсюда правило: не выполняй платные и необратимые
операции в узле, который прерывается. Вынеси списание в отдельный узел перед паузой или сделай саму операцию
идемпотентной по ключу, который не изменится при повторном запуске узла, —
[дисциплина ключа идемпотентности](../tool-use/deep-dive.md) из урока про инструменты, применённая
к собственному графу. Это тот самый HITL-узел из Части 1, опертый на реальное сохранение состояния. Общий
слой человека-в-цикле и бюджета разбирает урок про [планирование и циклы](../planning-loops/index.md).
...
- Устойчивое исполнение: чекпойнтер сохраняет состояние на каждом шаге с привязкой к `thread_id`, так что прогон
  ...
  Одобрения можно ждать часами, но после него прерванный узел исполняется ещё раз — с самого начала.
  Режимы `durability` (`exit` / `async` / `sync`) меняют скорость на надёжность, бэкенды сменные (память — для разработки,
  Postgres или Redis — для прода), а агенту без состояния на один проход не нужно ничего из этого.
...
**[Новые термины](../../glossary.md#orchestration-frameworks)**: state graph, checkpointer, checkpoint backend, thread (thread_id), durable execution, conditional edge, framework long-term memory (store), declarative vs imperative agent definition.

...
```mermaid
flowchart TD
    Root["Задача"] --> A["План A · 0.8"]
    Root --> B["План B · 0.3"]
    Root --> C["План C · 0.6"]
    B -. "отсекаем" .-> Bx["✗ низкая ценность"]
    A --> A1["Шаг · 0.9"]
    A --> A2["Шаг · 0.2 · тупик"]
    A2 -. "возврат" .-> C
    C --> C1["Шаг · 0.7 → продолжаем"]
```

*Поиск по дереву планов: модель порождает ветки, оценивает каждую баллом, разворачивает лучшие, отсекает
слабые и пятится из тупиков в соседнюю ветку.*
...
```mermaid
flowchart LR
    subgraph SR["Self-Refine — внутри одного прогона"]
        G["Ответ"] --> Fb["Самокритика"]
        Fb --> Rf["Правка"]
        Rf -. "цикл" .-> Fb
    end
    subgraph RX["Reflexion — между попытками"]
        At["Попытка"] --> Re["Рефлексия над провалом"]
        Re --> Buf[("Эпизодическая память")]
        Buf --> Nx["Следующая попытка её читает"]
    end
```
...
```mermaid
flowchart TB
    subgraph Main["Основной контекст · окно · RAM"]
        WM["Рабочая память:<br/>план · последние наблюдения"]
    end
    subgraph Ext["Внешний контекст · хранилище · диск"]
        EM["Эпизодическая / семантическая память:<br/>прошлые эпизоды · факты"]
    end
    WM -- "выгрузить · вытеснить" --> EM
    EM -- "подкачать по релевантности" --> WM
```
...
**[Новые термины](../../glossary.md#planning-loops)**: Tree of Thoughts (ToT), Graph of Thoughts (GoT), LATS, Self-Refine, Reflexion, plan search, episodic memory, semantic memory, virtual context management (MemGPT), trajectory evaluation, pass^k.

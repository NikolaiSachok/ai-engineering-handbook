...
**[Новые термины](../../glossary.md#mcp)**: MCP (Model Context Protocol), MCP server, MCP client, MCP resources, MCP prompts, M×N integration problem, A2A (Agent-to-Agent).

---

:::note[Дальше — вторая часть урока]

**[Серверы, транспорт и доверие](./deep-dive.md)** — как собрать MCP-сервер на практике, sampling и
elicitation в деталях, реестры и обнаружение MCP-серверов, выбор транспорта (stdio и streamable
HTTP) как решение о доверии, развивающийся ландшафт агентных протоколов (A2A и соседи) и укреплённые схемы развёртывания
недоверенных серверов.

См. также: [завершающий разбор реальных агентов](../real-agents.md), где MCP работает на живых Claude,
OpenAI и Gemini; [мультиагентные системы](../multi-agent/index.md) — ось агент ↔ агент, которую MCP
оставляет открытой; и [фреймворки оркестрации](../orchestration-frameworks/index.md) — как упаковать эти
подключения в библиотеку.

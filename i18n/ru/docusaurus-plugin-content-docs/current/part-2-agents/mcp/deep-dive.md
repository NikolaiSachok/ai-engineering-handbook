...
```mermaid
flowchart TB
    subgraph Trusted["Твоя машина — доверие по локальности"]
        subgraph Host["Хост · LLM-приложение"]
            CA["Клиент A"]
            CB["Клиент B"]
        end
        SL["Локальный сервер<br/>инструменты/ресурсы/промпты"]
        CA -->|"stdio"| SL
    end
    subgraph Untrusted["Сеть — граница доверия пересечена"]
        SR["Удалённый сервер<br/>инструменты/ресурсы/промпты"]
    end
    CB -->|"streamable HTTP"| SR
```

*Один хост держит несколько клиентов, у каждого связь один-к-одному с одним сервером по своему транспорту —
stdio к локальному подпроцессу, streamable HTTP к удалённому серверу по сети. «Доверие по локальности» — это
умолчание, а не гарантия: как напоминает раздел про транспорты, локальный stdio-сервер всё равно остаётся
чужим кодом с правами твоей машины.*
...
```mermaid
flowchart TB
    subgraph Untrusted["Недоверенный сервер"]
        R["ресурсы"]
        TR["результаты инструментов"]
        TD["описания инструментов"]
    end
    subgraph Boundary["Защита на границе"]
        AG["Одобрение человека"]
        SP["spotlighting"]
        LP["Наименьшие привилегии"]
        SB["Песочница"]
    end
    subgraph Trusted["Доверенная сторона"]
        H["Хост и его модель"]
        VT["Проверенные локальные инструменты"]
    end
...
**[Новые термины](../../glossary.md#mcp)**: MCP host, capability negotiation, roots, sampling, elicitation, streamable HTTP, MCP registry, server discovery, tool poisoning, rug pull, confused deputy.

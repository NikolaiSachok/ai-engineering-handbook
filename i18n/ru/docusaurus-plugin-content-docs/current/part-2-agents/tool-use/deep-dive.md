...
sidebar_label: "Надёжность и масштаб"
sidebar_position: 2
---
...
инструмента → продолжение. Здесь мы берём его же и разбираем глубже: что
происходит, когда модель за один ход выдаёт сразу несколько вызовов, как схема аргументов удерживается
...
За один ход модель может выдать не один вызов, а сразу **несколько независимых вызовов инструментов
...
flowchart TB
    ...
    R --> C["tool result ×3, собраны вместе"]
    C --> M2["Модель продолжает"]
```

*Fan-out: модель выдаёт три вызова разом. Fan-in: среда выполнения запускает их, собирает три
...
- **Anthropic Claude.** Параллельные вызовы включены по умолчанию — модели Claude 4 группируют вызовы,
  когда это оправдано. Выключается это флагом `disable_parallel_tool_use: true`, причём
...
```mermaid
flowchart TB
    S["JSON Schema"] --> G["Компилируем в грамматику"]
    G --> Step["На каждом шаге декодинга"]
    Step --> Cand["Токены-кандидаты"]
    Cand --> Mask["Маскируем невалидные токены"]
    Mask --> Sample["Сэмплируем валидный токен"]
    Sample --> Step
```
...
Строгий режим у провайдеров (имена — как есть):

- **OpenAI**: `strict: true` внутри определения функции — вызовы надёжно соответствуют схеме (в отличие от
  режима без гарантий); под капотом это **Structured Outputs** / строгий декодинг. Требования:
...
- **ошибка валидации** — аргументы правильно сформированы, но не проходят твои проверки (вне диапазона,
...
сбой. Повторять тот же самый вызов после детерминированного сбоя — значит жечь бюджет и деньги: он
...
Когда НЕ надо. Не повторяй детерминированный сбой неизменным. И не повторяй пишущий вызов с побочным
эффектом, который мог частично пройти, без гарантии идемпотентности (мостик в раздел об идемпотентности ниже). Повтор
...
[мультиагентные системы](../multi-agent/index.md), и это граница «когда не стоит и дальше растить одного агента».

Когда НЕ надо. Не заводи tool-RAG и динамический набор раньше времени. На горстке инструментов это лишняя
машинерия со своим собственным источником сбоев — сбоем поиска; самое простое, что работает, — полный
...
validation)** — механизм контроля. Строгий декодинг из второго раздела даёт правильно сформированные
...
Ошибка валидации — тоже промпт. Верни её модели как восстановимую, читаемую подсказку — уже
знакомый приём «ошибка — это промпт» из раздела про повторы, — чтобы модель исправила аргумент и повторила.
Тот же самозалечивающийся цикл, только теперь он стережёт границу *перед* исполнением, а не после сбоя.
...
- Параллельные вызовы: за один ход модель выдаёт несколько независимых вызовов, среда выполнения
...
**[Новые термины](../../glossary.md#tools)**: parallel tool calls, constrained decoding, strict mode / Structured Outputs, idempotency / idempotency key, tool-RAG / dynamic tool loadout, argument validation, retry budget.

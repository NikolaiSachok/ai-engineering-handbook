...
flowchart TB
    ...
    OCR --> Tables["Структура таблиц"]
    Tables --> Struct["Структурный текст: Markdown / JSON, иерархия сохранена"]
...
Одно структурное обстоятельство склоняет весь выбор в сторону качества: **парсинг офлайновый и разовый
для каждого документа**, как и вся остальная индексация. Он отрабатывает один раз, до любого запроса.
Поэтому здесь ты можешь позволить себе куда более дорогой парсер, чем когда-либо оправдал бы на запросе, —
его цена размазывается по всем будущим вопросам.

:::warning[Если парсер расплющил таблицу, ниже её уже никто не восстановит]

Парсинг питает структурную нарезку из Части 1 и её метаданные — путь по заголовкам и структуру таблиц.
Нарезка умеет уважать только ту структуру, которая пережила извлечение. Расплющи таблицу на этапе
парсинга — и ни стратегия нарезки, ни эмбеддинги, ни реранкер ниже уже не соберут столбцы обратно.
Решения по парсингу задают потолок всему, что способен выдать остальной слой.
...
```mermaid
flowchart LR
    subgraph Naive["Наивная нарезка"]
        direction LR
        N1["Сначала режем"] --> N2["Эмбеддинг каждого чанка порознь"]
    end
    subgraph Late["Late chunking"]
        direction LR
        L1["Эмбеддим весь документ"] --> L2["Потом режем + усредняем по чанку"]
    end
```
...
**[Новые термины](../../glossary.md#ingestion-chunking)**: document parsing / layout-aware extraction, OCR, late chunking, embedding fine-tuning, Matryoshka Representation Learning (MRL), contextual retrieval, multilingual embeddings, dimensionality.

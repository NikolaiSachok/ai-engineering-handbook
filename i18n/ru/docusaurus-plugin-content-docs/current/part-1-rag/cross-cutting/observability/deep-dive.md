...
[урок про экосистему инструментов](../../../part-3-production/tooling-ecosystem/index.md) Части III — и не учит
...
```mermaid
flowchart TB
    Q["Приходит запрос"] --> H["head-based сэмплирование<br/>оставить X% по trace-id<br/>(дёшево, решаем сразу)"]
    H --> C["Трейс завершается<br/>все спаны собраны"]
    C --> T{"tail / priority:<br/>ошибка? медленно? низкий балл eval?<br/>дизлайк? срабатывание guardrail?"}
    T -->|"да — интересный"| K["Оставить 100%"]
    T -->|"нет — рутинный"| D["Отбросить (или базовая ставка)"]
    K --> S["Хранилище трейсов"]
```
...
переингестом. Саму фиксацию версии модели (model pinning) как приём разбирает [LLMOps](../../../part-3-production/llmops/index.md)
...
```mermaid
flowchart TB
    M["Ряд во времени:<br/>качество / латентность / стоимость"] --> DR{"Статистически значимая просадка?"}
    DR -->|"да"| A["Локализовать по спанам трейса"]
    A --> Causes
    subgraph Causes["Возможные причины"]
        direction LR
        C1["Просели скоры retrieval?<br/>(переиндексация / смена индекса)"]
        C2["Сменился gen_ai.response.model?<br/>(поставщик выкатил другую версию)"]
        C3["Изменился шаблон промпта?<br/>(деплой)"]
        C4["Сместилось распределение входа?<br/>(новый тип запросов)"]
    end
    Causes --> CE["Сверить начало с событием-изменением"]
    CE --> F["Падавшие трейсы →<br/>новые кейсы golden set"]
    F --> E["Обратно в eval"]
```
...
**[Новые термины](../../../glossary.md#observability)**: head-based sampling, tail-based sampling, приоритетное

...
**[Новые термины](../../../glossary.md#evaluation)**: evaluation, golden set / golden dataset / ground truth, answer
...
**[Устройство метрик и калибровка судьи](./deep-dive.md)** — второй проход слоя оценки: как на самом деле
считаются метрики в духе Ragas (faithfulness, релевантность ответа, context precision и recall), как
калибруют LLM-судью по человеческим меткам и откуда берётся его предвзятость (позиционная, по длине,
самопредпочтение; pairwise против pointwise), и как собирают golden set руками — согласие между разметчиками
и активный сэмплинг.
...
[Guardrails](../guardrails/index.md) и [Observability](../observability/index.md).

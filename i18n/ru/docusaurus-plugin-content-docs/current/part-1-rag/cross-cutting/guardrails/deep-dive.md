...
```mermaid
flowchart TB
    U["Недоверенный текст:<br/>«Таким образом...»"] --> D["delimiting<br/>[[ Таким образом ]]<br/>только границы"]
    U --> M["datamarking<br/>«Таким^образом»<br/>маркер вместо каждого пробела"]
    U --> E["encoding<br/>base64 / ROT13<br/>больше не читается как инструкция"]
    D --> R["Модель читает это как данные,<br/>а не как инструкции"]
    M --> R
    E --> R
    R --> L["разделение сильнее ↑ · потеря качества ↑<br/>delimiting → datamarking → encoding"]
```

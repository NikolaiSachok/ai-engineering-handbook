...
```mermaid
flowchart TB
    Q["Запрос"] --> D["Плотный поиск"]
    Q --> B["Поиск BM25"]
    D --> LD["Ранжированный список (плотный)"]
    B --> LB["Ранжированный список (BM25)"]
    LD --> RRF["RRF: сумма 1/(k + rank)"]
    LB --> RRF
...
У маршрутизатора есть и ветки, которых этот урок не разбирает. За формулировкой «в каком индексе» тихо стоит допущение, что любая ветка заканчивается векторным поиском. Но для части вопросов нужен структурный путь: на агрегирующий вопрос выборка top-K физически ответить не может, а на вопрос к корпусу целиком не отвечает ни один отдельный чанк. Куда ведут такие ветки и стоит ли их вообще строить — предмет урока про [структурированные знания](../structured-knowledge/index.md), а маршрутизатор — то, что отправляет запрос по одной из них.
...
```mermaid
flowchart TB
    Q["Запрос"] --> Rt["Маршрутизация: какой индекс"]
    Rt --> Pf["Pre-filter: ACL и метаданные"]
    Pf --> AN["ANN-поиск"]
    AN --> Cand["Кандидаты"]
```
...
**[Новые термины](../../glossary.md#retrieval)**: score fusion / score normalization, LLM reranker, late interaction / ColBERT, multi-vector retrieval, contextual retrieval, query routing, pre-filter / post-filter.

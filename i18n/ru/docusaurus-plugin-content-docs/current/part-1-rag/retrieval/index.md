...
чанков. Поэтому вторая стадия работает на **точность**: cross-encoder из [прошлого урока](../ingestion/index.md)
...
Здесь контроль доступа читается как механика поиска, но решает он кое-что покрупнее. Фильтр может быть разным
для каждого пользователя, а веса модели у всех одни и те же. Отсюда архитектурный вывод: знания, доступ к
которым зависит от прав пользователя, нельзя зашивать в веса — им место за фильтром, см. [когда менять
веса](../../part-3-production/llmops/deep-dive.md).
...
MRR, nDCG. Метрики формализуем в слое [Evaluation](../cross-cutting/evaluation/index.md).
...
**[Новые термины](../../glossary.md#retrieval)**: retrieval failure / generation failure, dense retrieval,
...
**[Слияние, ранжирование и метрики](./deep-dive.md)** — второй проход слоя поиска: механика HyDE и когда
она вредит, внутренности гибридного слияния (взвешивание RRF и нормализация score), выбор реранкера
(cross-encoder vs LLM), parent-document и late-interaction (ColBERT) поиск, маршрутизация запросов по

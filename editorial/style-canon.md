...
- **`<course>.md`** — one **per-course term ledger** (`rag.md`, `ai-sdlc.md`, …): the term registry,
  ...
  extracted from it verbatim. A frozen ledger gets a **`<course>-addendum.md`** for post-freeze decisions —
  `rag-addendum.md` is the live continuation of `rag.md`; load both, never edit the frozen file.
...
|---|---|---|---|
| **RU + EN** | [`canon/ru/_language.md`](./canon/ru/_language.md) | [`canon/ru/rag.md`](./canon/ru/rag.md) (frozen) · [`canon/ru/rag-addendum.md`](./canon/ru/rag-addendum.md) (live continuation) · [`canon/ru/ai-sdlc.md`](./canon/ru/ai-sdlc.md) · [`canon/ru/design-scenarios.md`](./canon/ru/design-scenarios.md) | RU is audience-primary (written natively, never by translation); EN is the canonical source locale in `docs/`. **Design Scenarios is English-only** (`locales: ['en']`): its ledger sits in this folder because EN shares `_language.md` here, and it records **EN** decisions only — there is deliberately **no SK or DE ledger** for it. Its one non-English surface, the localized landing card, is recorded in a single scoped section of that same ledger. |
| **SK** | [`canon/sk/_language.md`](./canon/sk/_language.md) | [`canon/sk/rag.md`](./canon/sk/rag.md) (frozen) · [`canon/sk/rag-addendum.md`](./canon/sk/rag-addendum.md) (live continuation) · [`canon/sk/ai-sdlc.md`](./canon/sk/ai-sdlc.md) | Slovak — presentation-grade secondary locale, composed natively from EN meaning. Mirrors the RU/EN canon's *shape*, not its decisions; carries the anti-bohemism gate + EN→SK calque templates in `_language.md`. |
| **DE** | [`canon/de/_language.md`](./canon/de/_language.md) | [`canon/de/rag.md`](./canon/de/rag.md) · [`canon/de/ai-sdlc.md`](./canon/de/ai-sdlc.md) | German — **bootstrap canon, locale wired as *unreleased*** (built + link-gated in CI, gated out of the deployed dropdown). Presentation-grade secondary locale composed natively from EN meaning. Mirrors the SK canon's *shape* and **inverts its central decision**: German is English-led with a German article, the kept-EN register is several times larger, and the "an anglicism is a defect" gate is deliberately **not** ported. Adds two sections the Slavic canons do not need — **gender/plural/genitive for every kept-EN noun** and **Denglisch verb morphology** — plus Durchkopplung, regional variance (CH/AT) as the structural analogue of the anti-bohemism check, gendering, and syntactic EN→DE calque templates. Owner cannot check German (~A1), so every ruling carries a status token (SETTLED / WEAK / HOUSE / DISPUTED / OPEN) and a citable source. **Term ledgers settled 2026-07-28** (#275/#276): `_language.md` §1.1.a–g carries the full register (article · plural · genitive) plus **§1.1.g — the terms where the German form wins against the "German keeps English" instinct, which is the table to read before rendering anything**; both course ledgers carry a `constructions` section written as **conditions, not substitutions**, the fixed recurring block strings, and a glossary **anchor contract** (`\{#id}`, ordered id equality, backslash mandatory under `future.v4`). |
...
`canon/<lang>/<course>.md` for each language **the course actually declares** (`locales` in
`docusaurus.config.ts` is the source of truth) and update the row here; the shared `_language.md` is reused
unchanged. A course that declares `['en']` gets **one** ledger, in `canon/ru/` (the RU+EN folder) — do not
manufacture ledgers for locales the course will never have. Seed every new ledger with its book units, its
recurring block strings, its cross-course reference conventions, and a terms table whose rows start **ON
PROBATION**. Keep this router's filename stable — it is the fixed entry point.

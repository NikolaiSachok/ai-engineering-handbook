...
### Fáza 21 — vlna 3, lekcia Ekosystém nástrojov: rozhodnutia kánonu

Tretia lekcia vlny 3 (Časť III), stránky `index` + prehĺbenie „Vlastný hosting a prepojenie“. Plný
redakčný tím v synchrónnych dávkach (literárny, naivný jednojazyčný čitateľ, technický/fakt, korektor,
konzistenčný, riadiaci), adjudikácia (fidelita > štýl; glosár/kánon > novinka jednej stránky), oprava a
**studený prechod ×2** (čerstvá dvojica literárny + naivný čitateľ na každej stránke, zdroj skrytý).
Prezentačná lokalita bez ľudskej poistky — brány niesli plnú váhu.
...
**Smer termínu — glosár katalogizuje lemmu, telo vedie podľa headwordu; zrkadlí Fázu 18/19/20.**
- **kept-EN, EN-vedené (glos raz na stránku):** **safety classifier** „(bezpečnostný klasifikátor)“ (glosár
  headword — viď rezolúciu nižšie); **source-available** (vysvetlené v próze „zdrojový kód je dostupný… nie
  open source v zmysle OSI“, ELv2 = identifikátor licencie); **structured output** „(structured output)“
  (Trieda 2, most raz — index r. 48 dorobený); **instrumentation** „(inštrumentácia)“ (EN-vedený povrchový
  termín; sloveso „inštrumentovať“ + skloňovaná „inštrumentácia“ natívne — split vedome ponechaný); **OLTP**
  „(transakčné, riadok po riadku)“ / **OLAP** „(analytické, stĺpcové)“; **golden set** „(etalónová sada)“
  (glos raz aj na prehĺbení — r. 115 dorobený, Fáza 8/9 pravidlo). Holé kept-EN (§1.1): OTLP, CI, SaaS, UTC,
  MIT, ELv2, OSI, endpoint (korpusovo bežné), upsert (dorobený ľahký glos „(vloží alebo aktualizuje)“ pri
  prvom prozaickom výskyte — inak EN DB-sloveso bez slovenského ekvivalentu).
- **SK-vedené / naturalizované (most `(anglický originál)` raz):** **prevádzka u seba / prevádzkovať u seba
  (self-hosting)** — PRIMÁRNA prozaická forma (viď rezolúciu); **validátor (validator)**; **prepojenie /
  prepojiť (wiring)**; **exportér (exporter)** (Fáza 10 precedens); **jednotkový test (unit test)**; **fronta
  (queue)**; **spätná väzba (feedback loop)**; **prevádzková vrstva (operations layer)**; **pruh (lane)**;
  **stack (zostava nástrojov)** (ľahký glos raz); **ingestion (príjem dát)** — dorobený glos pri prvom
  prozaickom výskyte na prehĺbení (r. 28), inak kept-EN skloňované („ingestion cesta/API“); **prevádzkový
  postup (playbook)** — rozhodnuté SK-vedené (naivný nedekódoval holé „playbook“; „prevádzame prevádzkový
  playbook“ prepísané na „preberáme prevádzkový postup (playbook)“).
- **Nové vlastné mená do §1.1 (Trieda 1, kept-EN, nepreložené):** LangSmith, Guardrails AI, Guardrails Hub,
  Colang, OpenInference, OpenLLMetry, OpenTelemetry Collector, Bedrock Guardrails, Azure AI Content Safety,
  Vertex Model Armor, Valkey, Railway, Next.js, ClickHouse, PyRIT (už spomínané v Časti I ostávajú).
- **Book-part vs lesson-part (Fáza 3):** „Prvá časť príručky“ = RAG Časť I; „Prvá časť lekcie“ = index tejto
  2-dielnej lekcie. Refrén „Prvá časť lekcie“ (~14×) na prehĺbení **stíšený** — 3 výskyty variované
  („úvod lekcie“ / vypustená atribúcia) proti AI over-signpostingu; scaffold zostáva, monotónnosť zlomená.

**REZOLÚCIA „safety classifier“ (smer-kolísanie §1.0 — KONFLIKT z briefu vyriešený).** Index ho zaviedol
EN-vedené „safety classifier (bezpečnostný klasifikátor)“, ale takeaway (index r. 89) a obe prozaické výskyty
na prehĺbení (r. 57, 99) bežali SK-vedené „bezpečnostný klasifikátor“. **ZJEDNOTENÉ na kept-EN EN-vedené na
OBOCH stránkach** (zhoda s glosár-headwordom „Safety classifier (bezpečnostný klasifikátor)“ + footer):
index r. 50 (definujúci výskyt, ostal) a r. 89 „safety classifiery“; prehĺbenie r. 57 „safety classifiery
(bezpečnostné klasifikátory)“ (prvý výskyt na stránke = glos), r. 99 holé „safety classifier“. Množné číslo
skloňované ako korpusové kept-EN (safety classifiery, zrkadlí tracy/spanov). Opisné „klasifikačné modely /
klasifikátor“ ostáva ako parafráza (nie termín). **Grep: SK-vedený samostatný „bezpečnostný klasifikátor“ =
len ako zátvorkový glos; smer-kolísanie = 0.**

**REZOLÚCIA self-hosting (brief navrhoval „vlastný hosting“; usadené).**
- **Prozaická forma vedie „prevádzka u seba (self-hosting)“** na oboch stránkach (§1.2 + Fáza 20 handoff) —
  žiadna próza nevedie „vlastný hosting“.
- **„Vlastný hosting“ len ako krátky `sidebar_label` prehĺbenia + text odkazu poznámky prehĺbenia** (index
  r. 98) — **bajt-identické** (over: `sidebar_label: "Vlastný hosting a prepojenie"` == `**[Vlastný hosting a
  prepojenie](./deep-dive.md)**`). „vlastný hosting“ = label-only variant, NIKDY próza; nie je to §1.0
  smer-kolísanie, ale zámerný label-vs-próza split.

**Verb-by-object (§1.5):** univerzálne „vydať“ **grep = 0** na oboch (jediné „vydaní/vydaním“ = release, OK).
platforma **dodáva / obsluhuje / pokrýva**; model **ohodnotí podľa / počíta / generuje**; validátor **vráti**
(`PassResult`/`FailResult`); Postgres **prestal stíhať** (nie „zaškrtol“ = odškrtnúť, falošný priateľ);
worker **upsertne / načíta**; OTel **je spojivo**; „X ti dá / zaručí“ (nie „kúpiť“ = získať).

**Figúry — studený prechod ×2 (naivný dekódoval pri prvom kontakte = confirmed):**
- **POTVRDENÉ → usadené:** „splácame dlh z Prvej časti“ (deferred-promise), „momentky/momentka“ (snapshot,
  z cloud-platforms), „rázcestie „postaviť či kúpiť““ (make-or-buy), „hviezdička“ (caveat/asterisk),
  „koľajnice“ (dialogue rails/guardrails), „podložie“ (vendor-neutral substrate), „Zotri značku“ (strip the
  branding), „zavri dvere (fail closed) / prepustiť (fail open)“, **„spojivo“** (OTel = connective tissue),
  **„švy / šev / na dvoch švoch“** (seams — dekódované cez mermaid label „OTel — šev“), **„odovzdanie“**
  (trace→golden-set handoff; POZOR: §1.2 rezervuje „odovzdanie riadenia“ pre agentový control-handoff — tu
  „odovzdanie“ samo = dátový/etapový handoff, nová vedomá sémantika), „kadencia“ (red-team schedule; naivný
  borderline-PASS).
- **ZAMIETNUTÉ / opravené:** **„primitívum“** (noun — naivný dekódoval len s námahou) → **„základný prvok“**;
  **„uhol“** pre *angle/approach* (geometrický) → **„prístup“**; **„postavené na vstup“** (built-for) →
  **„zamerané na vstup“**; **„poddeď“** (subclass — NEexistuje slovom) → **„odvoď od“** (podst. „podtrieda“
  OK); **„logá“** ako metonymia pre názvy značiek (falošný priateľ s „logy“) → **„nech je značka akákoľvek“**.

**Nové kalkové pasce a falošní priatelia (žatva Ekosystému — do grep-zoznamov §1.3/§1.4):**
- **„zaškrtol“** pre *bottlenecked/choked* ✗ (= odškrtnúť) → **„prestal stíhať / sa zadrhol“**.
- **„rezidenciu“** holé pre *data residency* ✗ (číta sa ako obydlie) → **„rezidencia dát“** (Fáza 20 forma).
- **„príbeh“** pre *backup/upgrade story* ✗ (= naratív) → **„si žiada vlastné zálohovanie…“**.
- **„domáca schéma“** pre *house/in-house schema* ✗ (= domáca) → **„vlastná / interná schéma“**.
- **„biznisové obmedzenie“** ✗ (hovorové) → **„obchodné obmedzenie“**.
- **„pre škálu“** pre *for scale* ✗ → **„pre škálovanie / kvôli škálovaniu“**.
- **„oproti OTel“** pre *instrument against* ✗ (= versus) → **„voči OTel“**.
- **„naprieč tým, ktorý produkt“** (*across whichever product*) ✗ → **„nech nasadíš ktorýkoľvek produkt“**.
- **„Čo … umožňuje …, je X“** (pseudo-cleft, rodina „X je to, čo…“) ✗ → **„Skladať … umožňuje X“**.
- **„medzeru napíšeš“** (*write the gap*) ✗ → **„medzeru vyplníš“**.
- **„nástroj so SaaS ako prvou voľbou“** (stacked-prep *SaaS-first tool*) → **„nástroj, ktorého prvou voľbou
  je SaaS“**.
- **„zakladať si ho vedľa“** (*file it next to*, reflexívne „si“ → číta „zakladať si na niečom“ = honosiť sa)
  → **„zaraď/zaraďuj ho vedľa“** (bez „si“).
- **„config-driven / playbook / spotlighting / PII pipeline“ v cross-ref pointer-liste** (prehĺbenie „Kde
  žije teória“) — terse labely tém, ktoré **žijú v prehĺbení guardrails/observability** a tam sú glosované;
  glosuje sa len termín, ktorý lekcia vlastní (ASR áno; spotlighting/PII nie — sú cudzie tejto lekcii).

**KANON-ADJUDIKÁCIA (naivný vs korpus): „trace/tracing/span“ ostávajú kept-EN skloňované BEZ prozaického
glosu** — dvaja naivní (batch + studený) klopli na „span“ ako nedefinovaný atómový prvok a na holé prvé
výskyty „trace“/„attack success rate“ (r. 10 concept-list). **Prehlasované kánonom (Fáza 10):** trace/span
sú korpusovo usadené kept-EN (zavedené v prehĺbení Observability Časti I, sesterské cloud-platforms/serving
ich vedú holé); glos na TEJTO stránke by rozbil korpusovú konzistenciu. Naivný gate modeluje „bez
cudzojazyčnej záchrany“, nie „bez znalosti Časti I“ — v knihe Časť I predchádza. „attack success rate“ je
glosovaný v domácej sekcii (r. 54); „red-teaming“ glos presunutý na prvý výskyt (index r. 22).

**§8 polotučné + em-dash + straight-quote (potvrdenie Fázy 2/6/10/12–20).** Celovetné zvýraznenia žiadne.
...
`../../part-2-agents/{mcp/index,planning-loops/deep-dive,multi-agent/deep-dive,tool-use/index}.md`. Časť III
súrodenci SK → `../serving/index.md`, `../cloud-platforms/index.md`. V rámci lekcie `./index.md` /
`./deep-dive.md`; glosár `../../glossary.md`. **Nepreložený LLMOps → holá EN-fallback „../llmops/“ (bez
`.md`)** — retrofit na `../llmops/index.md` až po landingu (Fáza 11 konvencia). Externé produktové odkazy
(langfuse.com, ragas.io, opentelemetry.io…) ponechané ako v EN zdroji (precedens cloud-platforms).

**Pre lekciu LLMOps (odovzdanie — POSLEDNÁ lekcia):**
- Index r. 78 „ako táto slučka žije po vydaní“ je EXPLICITNE predmetom LLMOps — drž odkaz „../llmops/“,
  retrofit na `.md` pri landingu. Prehĺbenie r. 133: red-team behy ako CI/pre-release brány + sledovanie
  **attack success rate** ako regresnej metriky — LLMOps túto „red-team-cadence-as-regression“ rozvíja;
  **PyRIT** pochádza z prehĺbenia guardrails.
- **multi-cloud brána / LiteLLM** hĺbka patrí LLMOps (Fáza 20 handoff) — ekosystém ju NEspomína (over ✓),
  LLMOps ju rozvíja.
- Prenes term-smer: **safety classifier** kept-EN EN-vedené; **guardrails „(bezpečnostné mantinely)“** /
  **observability „(pozorovateľnosť)“** glos raz na stránku; **golden set „(etalónová sada)“**; **red-teaming
  „(útočné testovanie)“**; **attack success rate „(miera úspešnosti útokov)“**; **on-demand „(platba za
  tokeny)“** (Fáza 20 korpusový smer, ak sa dotkne cien); **prevádzka u seba (self-hosting)** v próze;
  **trace/span/tracing** kept-EN skloňované bez glosu (korpusovo usadené); **make-or-buy = „postaviť či
  kúpiť“**. Karta 1 (chyba vs zlyhanie), verb-by-object §1.5, em-dash metronóm + zákaz „— ,“ +
  straight-quote checkpoint + NBSP platia ďalej.
- **Reziduál na milestone vlny 3:** on-demand glos „(platba za tokeny)“ vs Serving „(na požiadanie)“ (Fáza 20
  KONFLIKT) sa v tejto lekcii nevyskytol — ostáva na milestone-retrofit Servingu, ekosystém nemení.

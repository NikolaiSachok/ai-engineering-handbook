...
The AI SDLC term set, seeded from the course skeletons — the native writers' term to-do list. Each SK
...
| Least privilege | the agent can reach exactly the resources its current task needs and no more (five distinct scopes: repo, CI token, cloud IAM, tool allow-list, network egress). | **REUSE** `princíp najnižších oprávnení (least privilege)` — owner ruling from the RAG canon (`rag.md` §1.0), do not reopen. |

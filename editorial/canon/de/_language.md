...
| **die Disziplin** | discipline *(eine befolgte Praxis)* | **NEU 2026-08-07 — belegt, nicht erfunden.** Korpusform für *discipline* im Sinn „befolgte Praxis": **sechs Belege**, zwei davon für genau diesen Referenten — „**die Disziplin aus dem Tool-Einsatz**" (`mcp/deep-dive.md`, `mcp/index.md`), dazu „die Disziplin der Evaluierung" (2×) und „dieselbe Disziplin wie in Agentic RAG" (2×). ⚠ **Nicht** durch *die Regel* ersetzen: *Regel* verengt eine befolgte Praxis auf eine einzelne Vorschrift. **Fehlergestalt zum Merken: ein Termurteil nach Gehör („*Disziplin* ist hier ein Anglizismus, das Wort meint Selbstdisziplin oder ein Studienfach") wurde von einem einzigen Grep widerlegt.** Bei einem Wort, das es im Deutschen wirklich gibt, entscheidet die **Belegzahl im eigenen Korpus**, nicht das Ohr — und schon gar nicht ein aus einer anderen Lokale übernommener Befund (im Slowakischen ist `disciplína` tatsächlich für den *Fachgebiet*-Sinn reserviert; das überträgt sich **nicht**) |
...
| `lets X do Y` — `lets the interrupt wait hours`, `lets the run continue` | „**lässt** die Unterbrechung stundenlang **warten**", „lässt den Durchlauf fortsetzen" | ⚠ **Hier ist nicht nur der Bau falsch, sondern der SINN.** Der `lassen`-Kausativ heißt im Deutschen zuerst **„X hält Y hin / X lässt Y warten"** (*jemanden warten lassen* = ihn hinhalten), **nicht** „X ermöglicht Y das Warten" — englisches *lets X do Y* führt ein flüssiges Modell direkt hinein. Zweiter Schaden: der Kausativ macht das **Akkusativobjekt zum Subjekt des Infinitivs**, verschiebt also den semantischen Träger der Handlung weg vom gemeinten Akteur. Aus dem Sinn neu bauen, mit **Verb nach Objekt** (§1.5): „**Dank** derselben Persistenz **kann** eine Unterbrechung stundenlang **andauern**." / „Die Persistenz **ermöglicht** es, …" — eine *Unterbrechung* **dauert an**, sie *wartet* nicht; ein *Durchlauf* **setzt an**, er wird nicht *fortgesetzt gelassen*. **Belegter Vorfall 2026-08-07** (`orchestration-frameworks/deep-dive.md`): „dieselbe Persistenz **lässt** einen Human-in-the-Loop stundenlang **warten** und danach fortsetzen" stand ausgeliefert im Korpus; der Blindrichter stufte das Risiko unaufgefordert als „sehr hoch" ein. Das ist die **Falscher-Sinn-Falle (§1.1, Falle 1) auf Konstruktionsebene** — die gefährlichste Klasse, weil der Satz grammatisch tadellos ist und trotzdem etwas anderes sagt |
...
- `lässt ... warten` / `lässt ... fortsetzen` / `lässt ... laufen` (englischer *lets X do Y*-Kausativ, wo *ermöglichen / dank … kann* gemeint ist — Falscher Sinn, siehe Tabelle)
- `resultiert in`
- `unterschiedlich als`
- `endet damit, ... zu`
